...
class SegmentoQBradesco extends SegmentoQGenerico
{
    ...
    {
        $inscricao = $this->tipoInscricaoPagador;
        $numeroInscricao = preg_replace('/[[:punct:]]/', '', $this->numeroInscricao);

        if ($inscricao == 1) {
            return substr($numeroInscricao, 0, -2) . '0000' . substr($numeroInscricao, 9);
        }

        return sprintf("%015d", $numeroInscricao);
    }
